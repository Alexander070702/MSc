...
def sample_action(network, grid, device):
    tensor_grid = grid_to_tensor(grid, device)  # Pass device to grid_to_tensor
...
def grid_to_tensor(grid, device):
    ...
    # Move tensor to the correct device (CPU or CUDA)
...
def main():
    ...
    # Window setup and display initialization
    SCREEN.fill(BLACK)
    pygame.display.update()
    pygame.time.delay(1000)  # Initial delay to ensure window appears
    ...
    while True:
        ...
        fall_speed = 0.5  # Adjusted for slower descent
        fall_time = 0
        run = True
        ...
        while run:
            ...
            if fall_time / 1000 >= fall_speed:  # Control the fall speed
                fall_time = 0
                current_piece.y += 1
            ...
            # Sample an action using GFlowNet-inspired agent
            ...
            if check_lost(locked_positions):
                ...
                run = False  # End the game if lost
