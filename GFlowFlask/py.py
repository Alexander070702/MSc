...
pygame.display.set_caption("Tetris with Educational GFlowNet-inspired Agent")
...
class TetrisNet(nn.Module):
    def __init__(self):
        ...
        # Increased network complexity for better learning
...
# Updated reward function for better guidance
def calculate_reward(rows_cleared, current_height, holes):
    return rows_cleared * 20 - (current_height * 0.3 + holes * 0.1)

# Epsilon-greedy action sampling for exploration
def sample_action(network, grid, device, epsilon=0.1):
    if random.random() < epsilon:
        # Random action for exploration
        return random.randint(0, 3), torch.tensor([0.25, 0.25, 0.25, 0.25])
    else:
        tensor_grid = grid_to_tensor(grid, device)
        action_probs = torch.softmax(network(tensor_grid), dim=-1)
...
# Display educational and reward info
def display_info_panel(surface, score, reward, action_probs):
    # Fill the info panel
    ...
    for i, line in enumerate(info_text):
        ...
        surface.blit(info_label, (SCREEN_WIDTH + 10, 90 + i * 20))

    # Display action probabilities
    actions = ["Left", "Right", "Rotate", "Down"]
    for i, prob in enumerate(action_probs.flatten()):
        label = font_small.render(f"{actions[i]}: {prob:.2f}", 1, GREEN)
        surface.blit(label, (SCREEN_WIDTH + 10, 220 + i * 20))

def main():
    net = TetrisNet()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    net = net.to(device)
    optimizer = optim.Adam(net.parameters(), lr=0.0005)  # Reduced learning rate
    score = 0
    SCREEN.fill(BLACK)
    ...
    # Initialize reward and action probabilities with default values to avoid unbound errors
    reward = 0.0
    action_probs = torch.tensor([0.25, 0.25, 0.25, 0.25])  # Default equal probabilities
    ...
    while True:
        ...
        while run:
            ...
            if current_time - last_action_time >= action_cooldown:
                action, action_probs = sample_action(net, grid, device)
            ...
            display_info_panel(SCREEN, score, reward, action_probs)
