...
def calculate_reward(rows_cleared, grid, current_height, moves_since_last_clear=0):
    # Large base reward for clearing rows
    row_reward = 300 * rows_cleared if rows_cleared > 0 else 0  # Clear rows yield highest reward

    # Small reward for lower heights, capped to avoid excessive reward for height alone
    height_reward = max(5, 100 - 3 * current_height)  # Higher weight for low heights, but always positive

    # Hole penalty (small penalty for each hole)
    hole_penalty = 5  # Start with a small base penalty
    for y in range(GRID_ROWS):
        row = grid[y]
        for x in range(GRID_COLUMNS):
            if row[x] == BLACK and any(grid[i][x] != BLACK for i in range(y)):
                hole_penalty += 2  # Small incremental penalty per hole

    # Progressive penalty for moves without clearing rows to discourage moving down aimlessly
    down_move_penalty = moves_since_last_clear * 2  # Increases penalty with each move without row clear

    # Calculate total reward, favoring row clears and lower heights
    reward = row_reward + height_reward - (0.05 * hole_penalty) - down_move_penalty

    # Ensure reward is positive for GFlowNet consistency
    reward = max(10, reward)  # Minimum reward to maintain positivity
    ...
    while True:
        ...
        while run:
            ...
            if change_piece:
                ...
                reward = calculate_reward(rows_cleared, grid, current_piece.y, sum(row.count(BLACK) == 0 for row in grid))
